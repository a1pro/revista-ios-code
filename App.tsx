 ...
 */
const saveLanguage = async (langKey: string): Promise<void> => {
  try {
 ...
 */
const loadLanguage = async (): Promise<string | null> => {
  try {
...
const applyLanguage = (langKey: string): string => {
  ...
  // Update axios header
  axios.defaults.headers.common['Accept-Language'] = i18nLang;
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      try {
        // 1. Initialize language (LOAD SAVED LANGUAGE FIRST)
        ...
        // 3. Set initial language header (already set in initializeLanguage)
        const initialLanguage = getCleanLanguage();
        axios.defaults.headers.common['Accept-Language'] = initialLanguage;
...
// Export helper functions for use in other files
export {getCleanLanguage, saveLanguage, loadLanguage, applyLanguage, initializeLanguage, LANGUAGE_KEY};